...
fn bench_insert(len: usize) -> (Arc<RwLock<BTreeMap<i64, i64>>>, Arc<Vec<i64>>, Duration) {
...
fn bench_insert_sorted(len: usize) -> (Arc<RwLock<BTreeMap<i64, i64>>>, Arc<Vec<i64>>, Duration) {
  let mut m = BTreeMap::new();
...
fn bench_get(m: &Arc<RwLock<BTreeMap<i64, i64>>>, d: &Arc<Vec<i64>>) -> Duration {
  let n = num_cpus::get();
  let chunk = d.len() / n;
